...
export async function middleware(req, res, next) {
  try {
    ...
    if (!data) {
      ...
      console.error(getCurrentDatetime(), `#${req._id}`, Date.now() - req._tm, 'M1', req.path, JSON.stringify(req.body), JSON.stringify(response))
      return
    }
    ...
    if (Number(operatorId) !== Number(req.query.operatorId)) {
      res.status(500).end()
      console.error(getCurrentDatetime(), `#${req._id}`, Date.now() - req._tm, 'M2', req.path, JSON.stringify(req.body))
      return
    }
    ...
    res.status(200).json(response).end()
    console.error(getCurrentDatetime(), `#${req._id}`, Date.now() - req._tm, 'M3', req.path, JSON.stringify(req.body), JSON.stringify(response))
    return
  } catch (e) {
